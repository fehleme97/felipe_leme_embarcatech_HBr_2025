...
float adc_to_temperature_celsius(uint16_t adc_value) {
    ...
    float temperature_celsius = 27.0f - (voltage - 0.706f) / 0.001721f;  // Equação fornecida
...
int main() {
    ...
    // Habilita e seleciona o sensor de temperatura interno
    adc_set_temp_sensor_enabled(true);  
    adc_select_input(ADC_TEMPERATURE_CHANNEL);  

    while (true) {
        uint16_t adc_value = adc_read();
        float temperature_celsius = adc_to_temperature_celsius(adc_value);
        ...
        snprintf(texto, sizeof(texto), "       %.F C", temperature_celsius);
